// Self-hosted Noto fonts, so every script a stage uses has glyphs (no tofu).
// Noto Serif covers Latin/Cyrillic/Greek/Vietnamese; the Noto Sans families
// cover the complex scripts. @fontsource ships unicode-range subsets, so the
// browser only downloads the scripts actually shown.
